...
AddEventHandler('esx_poolcleaner:GiveItem', function()
	...
	if Quantity >= 20 then
		...
		TriggerClientEvent('esx:showNotification', _source, 'Vous avez nettoyé ~g~x' .. amount .. ' piscine')
	end
	...
	SetTimeout(Config.Zones.Vente.ItemTime, function()
		...
		if PlayersVente[source] == true then
			...
			if Quantity < Config.Zones.Vente.ItemRemove then
				TriggerClientEvent('esx:showNotification', _source, '~r~Vous n\'avez plus de '..Config.Zones.Vente.ItemRequires_name..' à encaisser.')
